...
# 使x_train符合Embedding輸入要求:[送入樣本, 循環核時間展開步數]
# 此處整個數據集送入,送入樣本數為len(x_train);輸入1個字母出結果,循環核時間展開步數為1
x_train = np.reshape(x_train, (len(x_train), 1))
y_train = np.array(y_train)

model = tf.keras.Sequential([Embedding(5, 2), #5行2列的可訓練參數矩陣
                             SimpleRNN(3), #3個記憶體的循環層,可自行調整個數,記憶體個數越多,記憶力越好,但是佔用資源會更多
                             Dense(5, activation="softmax")]) #全連接層,實現輸出層y(t)的計算物
...
for i in range(preNum):
    ...
    alphabet = [[w_to_id[alphabet1]]] #把輸入的數字轉換為int
    # 使alphabet符合Embedding輸入要求:[送入樣本數, 循環核時間展開步數].
    # 此處驗證效果送入了1個樣本,送入樣本數為1;輸入1個字母出結果,所有循環時間展開數步數為1;
