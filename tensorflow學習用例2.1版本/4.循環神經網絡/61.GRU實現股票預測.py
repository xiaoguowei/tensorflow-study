# 在2014年有人簡化了LSTM結構,提出了GRU網絡

# GRU計算過程:
# 1.更新門 Z(t) = o( W(z)* [h(t-1), X(t)] )
# 2.重置門 r(t) = o( W(r)* [h(t-1), X(t)] )
# 3.記憶體 h(t) = (1 - Z(t)) * h(t-1) + Z(t) * (h(t)波浪號)
# 4.候選隱藏層 (h(t)波浪號) = tanh(W * [r(t) * h(t-1),X(t)])

############ TF描述GRU層 ##################
# tf.keras.layers.GRU(記憶體個數,return_sequences=是否返回輸出) # True 各時間步輸出h(t)  False(默認) 僅最後時間步輸出h(t)
9
import numpy as np
...
model = tf.keras.Sequential([GRU(80,return_sequences=True), #記憶體設定80個,每個時間步推送h(t)給下一層
                             Dropout(0.2),
                             GRU(100), #100個記憶體
                             Dropout(0.2),
