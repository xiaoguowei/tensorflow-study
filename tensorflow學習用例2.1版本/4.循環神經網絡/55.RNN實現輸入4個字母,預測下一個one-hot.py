...
y_train = [w_to_id['e'], w_to_id['a'], w_to_id['b'], w_to_id['c'],w_to_id['d']] #[4,0,1,2,3]
...
# 此處整個數據集送入,送入樣本數為len(x_train);輸入4個字母出結果,循環核時間展開步數為4;表示為獨熱碼有5個輸入特征,每個時間步輸入特征個數為5
...
for i in range(preNum):
    alphabet1 = input("input test alphabet:") #等待連續輸入 4個字母
    alphabet = [id_to_onehot[w_to_id[a]] for a in alphabet1] #把輸入的字母轉換為獨熱碼
    #使alphabet符合SimpleRNN輸入要求:[送入樣本數, 循環核時間展開步數, 每個時間步輸入特征個數].
    # 此處驗證效果送入了1個樣本,送入樣本數為1;輸入4個字母出結果,所有循環時間展開數步數為4;表示為獨熱碼有5個輸入特征,每個時間步輸入特征個數為5
