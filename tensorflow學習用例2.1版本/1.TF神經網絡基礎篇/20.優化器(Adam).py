...
#Adam,同時結合SGDM一階動量和RMSProp二階動量
# mt(一階動量) = B1 * m(t-1) + (1-B1) * gt(梯度)    #修正一階動量的偏差
# vt(二階動量)= B2 * V(step-1) + (1-B2) * ((gt)**2)    #修正二階動量的偏差
# nt = lr * (mt / (vt開根號)) = lr * mt = lr * B1 * m(t-1) + (1-B1) * gt(梯度) / B2 * V(step-1) + (1-B2) * ((gt)**2)
# w(t+1) = wt - nt  = wt - lr *( B1 * m(t-1) + (1-B1) * gt(梯度) / B2 * V(step-1) + (1-B2) * ((gt)**2))
########################################################################################
