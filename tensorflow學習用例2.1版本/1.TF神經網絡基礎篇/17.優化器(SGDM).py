...
#SGDM 在SGD基礎上增加一階動量
# mt(一階動量) = B * m(t-1) + (1-B) * gt(梯度)   vt(二階動量)= 1   #m(t-1)表示上一時刻的一階動量
# nt = lr * (mt / (vt開根號)) = lr * mt = lr * (B* m(t-1) + (1-B) * gt )
# w(t+1) = w(t+1) = wt - nt  = wt - lr * (B* m(t-1) + (1-B) * gt )
########################################################################################
...
for epoch in range(epoch):
    for step,(x_train,y_train) in enumerate(train_db): #batch級別的循環,每個step循環一個batch
        ...
        w1.assign_sub(lr * m_w) #參數w1自更新
        b1.assign_sub(lr * m_b) #參數b自更新
        ###################################################################
        #每個epoch,打印loss信息
