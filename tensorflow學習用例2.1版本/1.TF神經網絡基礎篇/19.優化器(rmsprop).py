...
#RMSProp 在SGD基礎上增加二階動量
# mt(一階動量) = gt(梯度)   vt(二階動量)= B * V(t-1) + (1-B) * ((gt)**2)
# nt = lr * (mt / (vt開根號)) = lr *  gt / (B * V(t-1) + (1-B) * ((gt)**2))開根號
# w(t+1) = w(t+1) = wt - nt  = wt - lr * gt / (B * V(t-1) + (1-B) * ((gt)**2))開根號
########################################################################################
...
for epoch in range(epoch):
    for step,(x_train,y_train) in enumerate(train_db): #batch級別的循環,每個step循環一個batch
        ...
        w1.assign_sub(lr * grads[0] / tf.sqrt(v_w)) #參數w1自更新
        b1.assign_sub(lr * grads[1] / tf.sqrt(v_b)) #參數b自更新
        ###################################################################
        #每個epoch,打印loss信息
