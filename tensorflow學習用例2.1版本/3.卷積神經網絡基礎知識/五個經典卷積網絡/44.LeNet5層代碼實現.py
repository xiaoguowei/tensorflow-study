...
class LeNet5(Model):
    def __init__(self): # 共5層網絡
        super(LeNet5,self).__init__()
        # 第一層
        self.c1 = Conv2D(filters=6, kernel_size=(5, 5), activation= "sigmoid") #C #6個5*5的卷積核,步長是1(默認)
        self.p1 = MaxPool2D(pool_size=(2, 2), strides=2) #P #使用最大池化,池化核是2*2,步長為2,那個年代沒有全零填充,所以默認不使用全零填充
        # 第二層
        self.c2 = Conv2D(filters=16, kernel_size=(5, 5), activation= "sigmoid") #C #16個5*5的卷積核,步長是1(默認)
